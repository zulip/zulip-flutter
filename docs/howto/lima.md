...
The main upstream presubmit checks -- the analyze script,
`flutter test` in `packages/flutter` and `examples/api`, the
docs build -- run in the VM as-is, with one piece of setup:
the analyze script's Kotlin check needs `ktlint` on the PATH,
which needs a JRE. Upstream CI pins ktlint 1.5.0 and
OpenJDK 21. In the VM, run:
