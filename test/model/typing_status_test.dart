...
void checkSetTypingStatusRequests(FakeApiConnection connection, List<(TypingOp, SendableNarrow)> requests) {
...
void main() {
  ...
  group('handle set typing status', () {
    ...
    void sendTypingNotification(FakeAsync async, SendableNarrow? narrow, {
    ...
    Future<void> prepareStartTyping(FakeAsync async) async {
      await prepare();
      sendTypingNotification(async, narrow);
    ...
    void checkTrailingTypingStoppedRequest(SendableNarrow narrow) {
      connection.prepare(json: {});
      addTearDown(() => checkTypingRequest(TypingOp.stop, narrow));
    }

    test('start typing repeatedly extends idle timer', () => awaitFakeAsync((async) async {
      // t = 0: Start typing. The idle timer is set to typingStoppedWaitPeriod.
      ...
      // t = 100: The idle timer is reset to typingStoppedWaitPeriod.
      sendTypingNotification(async, narrow);
      check(connection.lastRequest).isNull();
      check(async.pendingTimers).single;

      async.elapse(model.typingStoppedWaitPeriod - waitTime);
      // t = typingStoppedWaitPeriod:
      //   Since the timer was reset at t = 100, the "typing stopped" notice has
      //   not been sent yet.
      check(connection.lastRequest).isNull();
      check(async.pendingTimers).single;

      async.elapse(waitTime);
      // t = typingStoppedWaitPeriod + 100:
    ...
    test('start typing repeatedly does not resend "typing started" notices', () => awaitFakeAsync((async) async {
      ...
      while (timeElapsed <= model.typingStartedWaitPeriod) {
        // t <= typingStartedWaitPeriod: "Typing started" notices are throttled.
        sendTypingNotification(async, narrow, expectedRequestCount: 0);
        check(connection.lastRequest).isNull();
      ...
      checkTrailingTypingStoppedRequest(narrow);
    }));
    ...
    test('actively stopping typing cancels the idle timer', () => awaitFakeAsync((async) async {
      ...
      sendTypingNotification(async, null);
      checkTypingRequest(TypingOp.stop, narrow);
      check(async.pendingTimers).isEmpty();
    }));
    ...
    test('start typing in a different destination resets the idle timer', () => awaitFakeAsync((async) async {
      ...
      // t = 0: Start typing. The idle timer is set to typingStoppedWaitPeriod.
      sendTypingNotification(async, topicNarrow);
      ...
      sendTypingNotification(async, dmNarrow, expectedRequestCount: 2);
      ...
      //   Because the old timer has been canceled at t = 100,
    ...
    test('start typing in a different destination resets typing started wait timeout', () => awaitFakeAsync((async) async {
      ...
      // t = 0: Start typing. The typing started time is set to 0.
      sendTypingNotification(async, topicNarrow);
      checkTypingRequest(TypingOp.start, topicNarrow);
      ...
      while (timeElapsed <= model.typingStartedWaitPeriod) {
        ...
        sendTypingNotification(async, dmNarrow, expectedRequestCount: 0);
        check(connection.lastRequest).isNull();
      ...
      sendTypingNotification(async, dmNarrow);
      checkTypingRequest(TypingOp.start, dmNarrow);

      checkTrailingTypingStoppedRequest(dmNarrow);
    }));
  });
}
