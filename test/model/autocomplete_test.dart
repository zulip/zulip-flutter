    ...
    group('sorting results', () {
      ...
      group('compareChannelsByName', () {
        ...
        test("favor channel with name coming first", () async {
          ...
          check(compare('announce', 'BACKEND')).isLessThan(0);
          check(compare('backend', 'announce')).isGreaterThan(0);
          check(compare('BACKEND', 'announce')).isGreaterThan(0);

          // TODO(i18n): add locale-aware sorting
          // check(compare('čat', 'design')).isLessThan(0);
          // check(compare('design', 'čat')).isGreaterThan(0);
        });

        test('both have identical name -> favor none', () async {
          check(compare('announce', 'announce')).equals(0);
          check(compare('BACKEND', 'backend')).equals(0);

          // TODO(i18n): add locale-aware sorting
          // check(compare('čat', 'cat')).equals(0);
        });
      });
      ...
      group('ranking across signals', () {
        ...
        final channels = [
          ...
          eg.subscription(eg.stream(name: 'Y',
              isRecentlyActive: false, streamWeeklyTraffic: null),
          ...
          eg.stream(name: 'V', isRecentlyActive: true, streamWeeklyTraffic: null),
          ...
          // Runner-up by name.
          eg.stream(name: 'A', isRecentlyActive: false, streamWeeklyTraffic: null),
          // Next two are tied because no remaining criteria.
          eg.stream(name: 'B', isRecentlyActive: false, streamWeeklyTraffic: null),
          eg.stream(name: 'b', isRecentlyActive: false, streamWeeklyTraffic: null),
        ];
        for (final narrow in [
        ...
        test('DmNarrow: subscribed (unmuted > pinned) > recently active > weekly traffic > name', () {
          ...
          final channels = [
            ...
            // The rest are runners-up by not being subscribed to.
            // Runner-up by being recently active.
            eg.stream(name: 'W', isRecentlyActive: true, streamWeeklyTraffic: null),
            // Runner-up by having more weekly traffic.
            eg.stream(name: 'V', isRecentlyActive: false, streamWeeklyTraffic: 100),
            // Runner-up by having weekly traffic defined.
            eg.stream(name: 'U', isRecentlyActive: false, streamWeeklyTraffic: 10),
            // Runner-up by name.
            eg.stream(name: 'A', isRecentlyActive: false, streamWeeklyTraffic: null),
            // Next two are tied because no remaining criteria.
            eg.stream(name: 'B', isRecentlyActive: false, streamWeeklyTraffic: null),
