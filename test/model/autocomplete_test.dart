...
void main() {
  ...
  group('TopicLinkAutocompleteView', () {
    ...
    group('new-topic result', () {
      ...
      test('query has trailing space -> no new-topic result', () async {
        // The trailing space also filters out prefix matches like "Design";
        // see TopicLinkAutocompleteQuery.testTopic.
        await prepare(query: 'desig ', topics: ['Design']);
        check(view.results).isEmpty();
        await prepare(query: 'design ', topics: ['Design', 'Design docs']);
        check(view.results).single
          .which(isTopicResult(channel.streamId, eg.t('Design docs')));
      });
    });
