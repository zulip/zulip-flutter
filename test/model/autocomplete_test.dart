...
void main() {
  ...
  group('TopicLinkAutocompleteView', () {
    ...
    test('updates results when topics are loaded', () => awaitFakeAsync((async) async {
      final narrow = ChannelNarrow(1);
      final channel = eg.stream(streamId: 1);
      final store = eg.store(initialSnapshot:
        eg.initialSnapshot(streams: [channel]));
      final connection = store.connection as FakeApiConnection;
      final topic1 = eg.getChannelTopicsEntry(maxId: 20);
      final topic2 = eg.getChannelTopicsEntry(maxId: 10);
      connection.prepare(delay: Duration(milliseconds: 1),
        json: GetChannelTopicsResult(topics: [topic1, topic2]).toJson());

      final view = TopicLinkAutocompleteView.init(store: store, narrow: narrow,
        query: TopicLinkAutocompleteQuery('', channelName: channel.name));
      bool done = false;
      view.addListener(() { done = true; });
      ...
      check(view.results).deepEquals([
        ...
        isTopicResult(channel.streamId, topic1.name),
        isTopicResult(channel.streamId, topic2.name),
      ]);
    }));

    test('non-empty query, no channel option in the results', () async {
      final narrow = ChannelNarrow(1);
      final channel = eg.stream(streamId: 1);
      final store = eg.store(initialSnapshot:
        eg.initialSnapshot(streams: [channel]));
      final connection = store.connection as FakeApiConnection;
      final topic1 = eg.getChannelTopicsEntry(maxId: 20, name: 'First');
      final topic2 = eg.getChannelTopicsEntry(maxId: 10, name: 'Second');
      connection.prepare(json: GetChannelTopicsResult(
        topics: [topic1, topic2]).toJson());

      final view = TopicLinkAutocompleteView.init(store: store, narrow: narrow,
        query: TopicLinkAutocompleteQuery('first', channelName: channel.name));
      bool done = false;
      view.addListener(() { done = true; });
      ...
      await Future(() {});
      check(done).isTrue();
      check(view.results).single.which(isTopicResult(channel.streamId, topic1.name));
    });

    test("query doesn't exactly match any topic -> a new topic result is added to the results' start", () async {
      final narrow = ChannelNarrow(1);
      final channel = eg.stream(streamId: 1);
      final store = eg.store(initialSnapshot:
        eg.initialSnapshot(streams: [channel]));
      ...
      final view = TopicLinkAutocompleteView.init(store: store, narrow: narrow,
        query: TopicLinkAutocompleteQuery('fir', channelName: channel.name));
      bool done = false;
      view.addListener(() { done = true; });
      ...
      check(done).isTrue();
      check(view.results).deepEquals([
        isNewTopicResult(channel.streamId, eg.t('fir')),
        isTopicResult(channel.streamId, topic1.name),
      ]);

      view.query = TopicLinkAutocompleteQuery('first topic', channelName: channel.name);
      await Future(() {});
      check(view.results).single.which(
        isNewTopicResult(channel.streamId, eg.t('first topic')));
    });
  });
  ...
  group('TopicLinkAutocompleteQuery', () {
    ...
    test('testTopic: topic is included if name words match the query', () {
      ...
      doCheck('generals chats',        '',                      false);
    });

    group('ranking', () {
      final queryChannel = eg.stream();

      int rankOf(String queryStr, dynamic candidate) {
        final query = TopicLinkAutocompleteQuery(queryStr, channelName: queryChannel.name);
        final TopicLinkAutocompleteResult? result;
        switch(candidate) {
          case ZulipStream(:final streamId) when streamId == queryChannel.streamId:
            result = query.testChannel(channelId: queryChannel.streamId);
          case null:
            result = query.testNewTopic(channelId: queryChannel.streamId, store);
          case TopicName():
            result = query.testTopic(channelId: queryChannel.streamId, topic: candidate, store);
          default:
            throw StateError('invalid candidate');
        }
        // (i.e. throw here if it's not a match)
        return result!.rank;
      }
      ...
      test('full list of ranks', () async {
        store = eg.store();
        final topic = eg.t('some topic');
        final connection = store.connection as FakeApiConnection;
        connection.prepare(json: GetChannelTopicsResult(topics:
          [eg.getChannelTopicsEntry(maxId: 1, name: topic.apiName)]).toJson());
        // Fetch the channel topics so we can check if the query topic is new.
        await store.topics.getChannelTopics(queryChannel.streamId);
        check([
          rankOf('', queryChannel),    // channel
          rankOf('topics', null),      // query topic is new
