...
void main() {
  ...
  void fillWithMessages(List<Message> messages) {
    check(
      isSortedWithoutDuplicates(messages.map((m) => m.id).toList()),
    ).isTrue();
  ...
  void checkMatchesMessages(Iterable<Message> messages) {
    assert(
      Set.of(messages.map((m) => m.id)).length == messages.length,
    ...
    for (final message in messages) {
      ...
      switch (message) {
        ...
        case DmMessage():
          expectedLocatorMap[message.id] = DmNarrow.ofMessage(
            message,
  ...
  group('constructor', () {
    ...
    test('not empty', () {
      ...
      prepare(
        initial: UnreadMessagesSnapshot(
          count: 0,
          channels: [
            ...
            eg.unreadChannelMsgs(
              streamId: stream1.streamId,
              topic: 'b',
              unreadMessageIds: [3, 4],
            ),
            eg.unreadChannelMsgs(
              streamId: stream2.streamId,
              topic: 'b',
              unreadMessageIds: [5, 6],
            ),
            eg.unreadChannelMsgs(
              streamId: stream2.streamId,
              topic: 'c',
              unreadMessageIds: [7, 8],
            ),
          ...
          huddles: [
            UnreadHuddleSnapshot(
              userIdsString: '1,2,${eg.selfUser.userId}',
              unreadMessageIds: [15, 16],
            ),
            UnreadHuddleSnapshot(
              userIdsString: '2,3,${eg.selfUser.userId}',
              unreadMessageIds: [17, 18],
            ),
          ],
          mentions: [6, 14, 18],
          oldUnreadsMissing: false,
  ...
  group('count helpers', () {
    ...
    test('countInTopicNarrow', () {
      ...
      final messages = [
        ...List.generate(
          7,
          (i) => eg.streamMessage(stream: stream, topic: 'a', flags: []),
        ),
        ...List.generate(
          2,
          (i) => eg.streamMessage(stream: stream, topic: 'A', flags: []),
    ...
    test('countInDmNarrow', () {
      ...
      fillWithMessages(
        List.generate(
          5,
          (i) => eg.dmMessage(from: eg.otherUser, to: [eg.selfUser], flags: []),
        ),
      );
    ...
    group('countInMentionsNarrow', () {
      ...
      test(
        'not affected by channel being unknown/unsubscribed/muted, nor topic being muted',
        () async {
          prepare();
          ...
          await store.addSubscription(
            eg.subscription(channel4, isMuted: false),
          );
    ...
    group('countInNarrowIncludingMuted', () {
      test('CombinedFeedNarrow includes muted topics and DMs', () async {
        final stream1 = eg.stream();
        final stream2 = eg.stream();
        prepare();
        ...
        // Visible count (excluding muted)
        check(model.countInNarrow(const CombinedFeedNarrow())).equals(2);
        // Total count (including muted)
        check(
          model.countInNarrowIncludingMuted(const CombinedFeedNarrow()),
        ).equals(5);
      });

      test('ChannelNarrow includes muted topics', () async {
        final stream = eg.stream();
        prepare();
        await store.addStream(stream);
        await store.addSubscription(eg.subscription(stream));
        ...
        // Visible count (excluding muted topic)
        check(model.countInNarrow(ChannelNarrow(stream.streamId))).equals(2);
        // Total count (including muted topic)
        check(
          model.countInNarrowIncludingMuted(ChannelNarrow(stream.streamId)),
        ).equals(3);
      });

      test(
        'TopicNarrow same as regular count (no muting within topic)',
        () async {
          final stream = eg.stream();
          prepare();
          await store.addStream(stream);
          ...
          final narrow = TopicNarrow(stream.streamId, eg.t('a'));
          check(model.countInNarrow(narrow)).equals(2);
          check(model.countInNarrowIncludingMuted(narrow)).equals(2);
        },
      );

      test('MentionsNarrow includes mentions in muted topics', () async {
        final stream = eg.stream();
        prepare();
        await store.addStream(stream);
        await store.addSubscription(eg.subscription(stream));
        ...
        // Visible count (topic 'a' is muted, but mentions still show)
        check(model.countInNarrow(const MentionsNarrow())).equals(2);
        // Total count (same, as mentions always show regardless of muting)
        check(
          model.countInNarrowIncludingMuted(const MentionsNarrow()),
        ).equals(2);
      });
    });
  });
  ...
  group('isUnread', () {
    ...
    doTestCommon(
      ...
      unreadChannelMessage.id,
      expected: true,
    );
    doTestCommon(
      'read channel message',
      readChannelMessage.id,
      expected: false,
    );

    doTestOldUnreadsMissing(
      'unread DM message',
      unreadDmMessage.id,
      expected: true,
    );
    ...
    group('stream messages', () {
      group('new unread follows existing unread', () {
        ...
        for (final (oldStream, newStream, oldTopic, newTopic) in [
          ...
          final description = [
            oldStream.streamId == newStream.streamId
                ? 'same stream'
                : 'different stream',
            oldTopic == newTopic ? 'same topic' : 'different topic',
          ].join(' / ');
          test(description, () {
            final oldMessage = eg.streamMessage(
              stream: oldStream,
              topic: oldTopic,
              flags: [],
            );
      ...
      test('topics case-insensitive but case-preserving', () {
        ...
        check(model).streams.values.single.entries.single
          ..key.equals(eg.t('aaa'))
    ...
    group('DM messages', () {
      ...
      group('new unread follows existing unread', () {
        for (final (oldDesc, oldFrom, oldTo) in variousDms) {
          ...
          final oldNarrow = DmNarrow.ofMessage(
            oldMessage,
            selfUserId: eg.selfUser.userId,
          );
          for (final (newDesc, newFrom, newTo) in variousDms) {
            final newMessage = eg.dmMessage(
              from: newFrom,
              to: newTo,
              flags: [],
            );
            final newNarrow = DmNarrow.ofMessage(
              newMessage,
              selfUserId: eg.selfUser.userId,
            );

            test(
              'existing in $oldDesc narrow; new in ${oldNarrow == newNarrow ? 'same narrow' : 'different narrow ($newDesc)'}',
              () {
                prepare();
                fillWithMessages([oldMessage]);
                model.handleMessageEvent(eg.messageEvent(newMessage));
  ...
  group('handleUpdateMessageEvent', () {
      ...
      for (final isKnownToModel in [true, false]) {
        for (final isRead in [false, true]) {
          ...
          for (final (messageDesc, message) in <(String, Message)>[
            ...
            test(
              '${isRead ? 'read' : 'unread'} $messageDesc message${isKnownToModel ? '' : ' (but read state unknown to model)'}',
              () {
                prepare();
                fillWithMessages([if (isKnownToModel) message]);
                ...
                for (final List<MessageFlag> newFlags in [
                  ...
                  if (!isKnownToModel) {
                    check(
                      because: "no crash if message is in model's blindspots",
                  ...
                  if (isRead ||
                      ...
                      (message.flags.contains(MessageFlag.mentioned) ||
                              message.flags.contains(
                                MessageFlag.wildcardMentioned,
                              )) ==
                          (newFlags.contains(MessageFlag.mentioned) ||
                              newFlags.contains(
                                MessageFlag.wildcardMentioned,
                              )))) {
                    checkNotNotified();
    ...
    group('moves', () {
      ...
      Future<void> prepareStore() async {
        ...
        unreadMessages = List<StreamMessage>.generate(
          10,
          (_) => eg.streamMessage(stream: origChannel, topic: origTopic),
        );
        readMessages = List<StreamMessage>.generate(
          10,
          (_) => eg.streamMessage(
            stream: origChannel,
            topic: origTopic,
      ...
      List<StreamMessage> copyMessagesWith(
        ...
        return messages
            .map(
              (message) => StreamMessage.fromJson(
                message.toJson()
      ...
      test(
        'moved messages ∩ unread messages ≠ Ø, moved messages ∩ read messages ≠ Ø, moved messages ⊅ unread messages',
        () async {
          await prepareStore();
      ...
      test('moved messages ⊃ unread messages', () async {
        ...
        checkNotifiedOnce();
        checkMatchesMessages(
          copyMessagesWith(unreadMessages, newTopic: newTopic),
        );
        final newMessageIds =
      ...
      test('moving to unsubscribed channels drops the unreads', () async {
        ...
        assert(!store.subscriptions.containsKey(unsubscribedChannel.streamId));
        fillWithMessages(unreadMessages);
      ...
      test('tolerates unsorted messages', () async {
        await prepareStore();
        final unreadMessages = List.generate(
          10,
          (i) => eg.streamMessage(stream: origChannel, topic: origTopic),
        );
        fillWithMessages(unreadMessages);
      ...
      test('message edit but no move', () async {
        ...
        model.handleUpdateMessageEvent(
          eg.updateMessageEditEvent(unreadMessages.first),
        );
        checkNotNotified();
        checkMatchesMessages(unreadMessages);
      });
  ...
  group('handleDeleteMessageEvent', () {
    ...
    final message2 = eg.dmMessage(
      id: 2,
      from: eg.otherUser,
      to: [eg.selfUser],
      flags: [],
    );
    final message3 = eg.dmMessage(
      id: 3,
      from: eg.thirdUser,
      to: [eg.selfUser],
      flags: [],
    );
    final message4 = eg.dmMessage(
      id: 4,
      from: eg.otherUser,
      to: [eg.selfUser, eg.thirdUser],
      flags: [],
    );
    final message5 = eg.dmMessage(
      id: 5,
      from: eg.otherUser,
      to: [eg.selfUser, eg.user(userId: 456)],
      flags: [],
    );

    final message6 = eg.dmMessage(
      id: 6,
      from: eg.selfUser,
      to: [],
      flags: [MessageFlag.mentioned],
    );
    final message7 = eg.dmMessage(
      id: 7,
      from: eg.otherUser,
      to: [eg.selfUser],
      flags: [MessageFlag.mentioned],
    );
    final message8 = eg.dmMessage(
      id: 8,
      from: eg.thirdUser,
      to: [eg.selfUser],
      flags: [MessageFlag.mentioned],
    );
    final message9 = eg.dmMessage(
      id: 9,
      from: eg.otherUser,
      to: [eg.selfUser, eg.thirdUser],
      flags: [MessageFlag.mentioned],
    );
    final message10 = eg.dmMessage(
      id: 10,
      from: eg.otherUser,
      to: [eg.selfUser, eg.user(userId: 456)],
      flags: [MessageFlag.mentioned],
    ...
    final message11 = eg.streamMessage(
      id: 11,
      stream: stream1,
      topic: 'a',
      flags: [],
    );
    final message12 = eg.streamMessage(
      id: 12,
      stream: stream1,
      topic: 'a',
      flags: [MessageFlag.mentioned],
    );
    final message13 = eg.streamMessage(
      id: 13,
      stream: stream2,
      topic: 'b',
      flags: [],
    );
    final message14 = eg.streamMessage(
      id: 14,
      stream: stream2,
      topic: 'b',
      flags: [MessageFlag.mentioned],
    ...
    test('bulk deletes of unreads', () {
      ...
      model.handleDeleteMessageEvent(
        DeleteMessageEvent(
          id: 0,
          messageIds: [11, 12],
          messageType: MessageType.stream,
      ...
      checkNotifiedOnce();
      checkMatchesMessages(
        expectedRemainingMessages..removeAll([message11, message12]),
      );
      model.handleDeleteMessageEvent(
        DeleteMessageEvent(
          id: 0,
      ...
      checkNotifiedOnce();
      checkMatchesMessages(
        expectedRemainingMessages..removeAll([message13, message14]),
      );
      model.handleDeleteMessageEvent(
        DeleteMessageEvent(
          id: 0,
  ...
  group('handleUpdateMessageFlagsEvent', () {
    final irrelevantFlags = MessageFlag.values.where(
      (flag) => switch (flag) {
    ...
    for (final (isRead, isMentioned) in [
      ...
      test(
        'remove irrelevant flags; ${isRead ? 'read' : 'unread'} / ${isMentioned ? 'mentioned' : 'not mentioned'}',
        () {
          ...
          for (final flag in irrelevantFlags) {
            model.handleUpdateMessageFlagsEvent(
    ...
    for (final (isRead, isMentioned) in [
      ...
      // the model, so we get coverage for that case too.
      test(
        'add irrelevant flags; ${isRead ? 'read' : 'unread'} / ${isMentioned ? 'mentioned' : 'not mentioned'}',
        () {
          final message = eg.streamMessage(
            flags: [
              if (isRead) MessageFlag.read,
      ...
      test('remove flag: ${mentionFlag.name}', () {
        final messages = <Message>[
          ...
          eg.dmMessage(
            from: eg.otherUser,
            to: [eg.selfUser],
            flags: [mentionFlag, MessageFlag.read],
          ),
        ];
