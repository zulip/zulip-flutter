...
''');
  ...
  group('mention', () {
    ...
    test('wildcard', () {
      ...
      check(wildcardMention(Wildcard.all, store: store()))
        .equals('@**all**');
      check(wildcardMention(Wildcard.everyone, store: store()))
        .equals('@**everyone**');
      check(wildcardMention(Wildcard.channel, store: store()))
        .equals('@**channel**');
      check(wildcardMention(Wildcard.stream,
          store: store(zulipFeatureLevel: 247)))
        .equals('@**channel**');
      check(wildcardMention(Wildcard.stream,
          store: store(zulipFeatureLevel: 246)))
        .equals('@**stream**');
      check(wildcardMention(Wildcard.topic, store: store()))
        .equals('@**topic**');
    });
  });
