...
''');
  ...
  group('channel link', () {
    test('channels with normal names', () async {
      ...
      check(channelLink(channels[3], pendingTopicAutocomplete: true, store: store)).equals('#**api_v3>');
      check(channelLink(channels[4], pendingTopicAutocomplete: true, store: store)).equals('#**build+test>');
      check(channelLink(channels[5], pendingTopicAutocomplete: true, store: store)).equals('#**init()>');
    });
    ...
    test('channels with names containing avoided characters', () async {
      ...
      check(channelLink(channels[4 - 1], pendingTopicAutocomplete: true, store: store))
        .equals('[#R&amp;D](#narrow/channel/4-R.26D)>');
      check(channelLink(channels[5 - 1], pendingTopicAutocomplete: true, store: store))
        .equals('[#UI &#91;v2&#93;](#narrow/channel/5-UI-.5Bv2.5D)>');
      check(channelLink(channels[6 - 1], pendingTopicAutocomplete: true, store: store))
        .equals('[#Save &#36;&#36;](#narrow/channel/6-Save-.24.24)>');
    });
  });
  ...
  group('topic link', () {
    ...
    test('channel or topic with names containing avoided characters', () async {
      ...
      check(topicLink(channels[4 - 1], topics[4 - 1], store: store))
        .equals('[#A&#42; > Save &#36;&#36;](#narrow/channel/4-A.2A/topic/Save.20.24.24)');
    });
  });
