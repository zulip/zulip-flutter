...
void main() {
  ...
  Future<void> prepareOutboxMessages({
    ...
    String topicStr = 'some topic',
  }) async {
    for (int i = 0; i < count; i++) {
      connection.prepare(json: SendMessageResult(id: 1).toJson());
      await store.sendMessage(
        destination: StreamDestination(stream.streamId, eg.t(topicStr)),
        content: 'content');
    }
  }
  ...
  group('fetchInitial', () {
    ...
    test('only outbox messages found', () => awaitFakeAsync((async) async {
      ...
      await prepareOutboxMessages(count: 1, stream: stream, topicStr: 'topic');
  ...
  group('MessageEvent', () {
    ...
    test('when no matching localMessageId is found ', () => awaitFakeAsync((async) async {
      ...
      await prepareOutboxMessages(count: 5, stream: stream, topicStr: 'other');
  ...
  group('removeOutboxMessageIfExists', () {
    final stream = eg.stream();
  ...
  group('notifyListenersIfOutboxMessagePresent', () {
    ...
    test('message not present', () => awaitFakeAsync((async) async {
      ...
      await prepareOutboxMessages(count: 5,
        stream: stream, topicStr: 'other topic');
...
void checkInvariants(MessageListView model) {
  ...
  for (final message in allMessages) {
    ...
    if (message is! MessageBase<StreamRecipient>) continue;
    final recipient = message.recipient;
    switch (model.narrow) {
      case CombinedFeedNarrow():
        check(model.store.isTopicVisible(recipient.streamId, recipient.topic))
          .isTrue();
      case ChannelNarrow():
        check(model.store.isTopicVisibleInStream(recipient.streamId, recipient.topic))
          .isTrue();
      case TopicNarrow():
      case DmNarrow():
      case MentionsNarrow():
