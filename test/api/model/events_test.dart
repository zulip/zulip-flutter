...
void main() {
  ...
  test('delete_message: require streamId and topic for stream messages', () {
    ...
    for (final json in [baseJsonStream, baseJsonChannel]) {
      check(() => DeleteMessageEvent.fromJson({
        ...json
      })).throws<void>();

      check(() => DeleteMessageEvent.fromJson({
        ...json, 'stream_id': 1, 'topic': 'some topic',
      })).returnsNormally();
  ...
  group('typing status event', () {
    ...
    test('stream/channel type missing streamId/topic', () {
      ...
      for (final json in [baseJsonStream, baseJsonChannel]) {
        check(() => TypingEvent.fromJson({
          ...json, 'stream_id': 123, 'topic': 'foo'}))
          .returnsNormally();
        check(() => TypingEvent.fromJson({
          ...json})).throws<void>();
        check(() => TypingEvent.fromJson({
          ...json, 'topic': 'foo'})).throws<void>();
        check(() => TypingEvent.fromJson({
          ...json, 'stream_id': 123})).throws<void>();
      }
    });
