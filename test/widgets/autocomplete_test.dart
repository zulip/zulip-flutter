...
void main() {
  ...
  group('@-mentions', () {
    ...
    void checkWildcardShown(Wildcard wildcard, PerAccountStore store, {
      required bool expected,
    }) {
      final richTextFinder = find.textContaining(wildcard.name, findRichText: true);
    ...
    testWidgets('wildcard options appear, disappear, and change correctly', (tester) async {
      ...
      checkWildcardShown(Wildcard.channel, store, expected: true);
      checkWildcardShown(Wildcard.topic, store, expected: true);
      checkWildcardShown(Wildcard.all, store, expected: false);
      checkWildcardShown(Wildcard.everyone, store, expected: false);
      checkWildcardShown(Wildcard.stream, store, expected: false);

      // Finishing autocomplete updates compose box; causes options to disappear
      await tester.tap(find.textContaining(Wildcard.channel.name,
        findRichText: true));
      await tester.pump();
      check(tester.widget<TextField>(composeInputFinder).controller!.text)
        .contains(wildcardMention(Wildcard.channel, store: store));
      checkWildcardShown(Wildcard.channel, store, expected: false);
      checkWildcardShown(Wildcard.topic, store, expected: false);
      checkWildcardShown(Wildcard.all, store, expected: false);
      checkWildcardShown(Wildcard.everyone, store, expected: false);
      checkWildcardShown(Wildcard.stream, store, expected: false);
