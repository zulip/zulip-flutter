...
Future<Finder> setupToComposeInput(
  WidgetTester tester, {
...
Future<Finder> setupToTopicInput(
  WidgetTester tester, {
  ...
  await testBinding.globalStore.add(
    eg.selfAccount,
    eg.initialSnapshot(realmEmptyTopicDisplayName: realmEmptyTopicDisplayName),
  );
  ...
  await tester.pumpWidget(
    TestZulipApp(
      accountId: eg.selfAccount.id,
      child: MessageListBlockPage(initNarrow: ChannelNarrow(stream.streamId)),
    ),
  );
  await tester.pumpAndSettle();
...
void main() {
  ...
  group('@-mentions', () {
    Finder findAvatarImage(int userId) => find.byWidgetPredicate(
      (widget) => widget is AvatarImage && widget.userId == userId,
    );
    ...
    testWidgets('user options appear, disappear, and change correctly', (
      tester,
    ) async {
      final user1 = eg.user(
        userId: 1,
      ...
      await tester.pump();
      check(
        tester.widget<TextField>(composeInputFinder).controller!.text,
    ...
    void checkWildcardShown(
      WildcardMentionOption wildcard, {
      required bool expected,
    }) {
      check(find.text(wildcard.canonicalString)).findsExactly(expected ? 1 : 0);
    }

    testWidgets('wildcard options appear, disappear, and change correctly', (
      tester,
    ) async {
      final composeInputFinder = await setupToComposeInput(
        tester,
      ...
      // Finishing autocomplete updates compose box; causes options to disappear
      await tester.tap(
        find.text(WildcardMentionOption.channel.canonicalString),
      );
      await tester.pump();
      check(
        tester.widget<TextField>(composeInputFinder).controller!.text,
    ...
    group('sublabel', () {
      Finder findLabelsForItem({required Finder itemFinder}) {
        final itemColumn = find
            .ancestor(of: itemFinder, matching: find.byType(Column))
            .first;
        return find.descendant(of: itemColumn, matching: find.byType(Text));
      }

      testWidgets('no sublabel when delivery email is unavailable', (
        tester,
      ) async {
        ...
        check(
          findLabelsForItem(itemFinder: find.text(user.fullName)),
        ).findsOne();

        debugNetworkImageHttpClientProvider = null;
      });

      testWidgets('show sublabel when delivery email is available', (
        tester,
      ) async {
        final user = eg.user(
          fullName: 'User One',
          deliveryEmail: 'email1@email.com',
        );
        final composeInputFinder = await setupToComposeInput(
          tester,
          users: [user],
      ...
      testWidgets('show sublabel for wildcard mention items', (tester) async {
        ...
        check(
          findLabelsForItem(itemFinder: find.text('channel')),
        ).findsExactly(2);
  ...
  group('#channel link', () {
    void checkChannelShown(ZulipStream channel, {required bool expected}) {
      check(
        find.ancestor(
          of: find.byIcon(iconDataForStream(channel)),
          matching: find.ancestor(
            of: find.text(channel.name),
    ...
    testWidgets('options appear, disappear, and change correctly', (
      tester,
    ) async {
  ...
  group('emoji', () {
    void checkEmojiShown(ExpectedEmoji option, {required bool expected}) {
      ...
      switch (display) {
        ...
        case ImageEmojiDisplay():
          displaySubject = check(
            findNetworkImage(display.resolvedUrl.toString()),
          );
    ...
    testWidgets('scroll position resets when query changes', (tester) async {
      ...
      final composeInputFinder = await setupToComposeInput(tester);
      store.setServerEmojiData(
        ServerEmojiData(codeToNames: scrollTestCodesToNames),
      );
    ...
    testWidgets(
      'scroll position is retained when text changes but query does not',
      (tester) async {
        final composeInputFinder = await setupToComposeInput(tester);
        store.setServerEmojiData(
          ServerEmojiData(codeToNames: scrollTestCodesToNames),
        ...
        await tester.pump();
        checkScrollItemsVisible(
          first: false,
          last: true,
        ); // scroll was NOT reset
  ...
  group('TopicAutocomplete', () {
    testWidgets('options appear, disappear, and change correctly', (
      ...
      final topicInputFinder = await setupToTopicInput(
        tester,
      ...
      await tester.pumpAndSettle();
      check(
        tester.widget<TextField>(topicInputFinder).controller!.text,
    ...
    testWidgets('text selection is reset on choosing an option', (
      tester,
    ) async {
      ...
      check(controller.value)
        ..text.equals('some')
        ..selection.equals(const TextSelection(baseOffset: 1, extentOffset: 3));
    ...
    testWidgets('display realmEmptyTopicDisplayName for empty topic', (
      tester,
    ) async {
    ...
    testWidgets('match realmEmptyTopicDisplayName in autocomplete', (
      tester,
    ) async {
      final topic = eg.getChannelTopicsEntry(name: '');
      final topicInputFinder = await setupToTopicInput(
    ...
    testWidgets(
      'autocomplete to realmEmptyTopicDisplayName sets topic to empty string',
      (tester) async {
        final topic = eg.getChannelTopicsEntry(name: '');
        final topicInputFinder = await setupToTopicInput(
          tester,
        ...
        final controller = tester
            .widget<TextField>(topicInputFinder)
            .controller!;
