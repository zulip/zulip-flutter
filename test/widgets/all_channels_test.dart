...
void main() {
  ...
  Future<void> setupAllChannelsPage(WidgetTester tester, {
    ...
    await store.addUserGroups([groupWithSelf, groupWithoutSelf]);
    ...
    check(find.byType(AllChannels)).findsOne();
  ...
  Future<ZulipStream> addPrivateChannelWithoutContentAccess(String? name) async {
    final channel = eg.stream(
      name: name,
      inviteOnly: true,
      canSubscribeGroup: groupSettingWithoutSelf,
      canAddSubscribersGroup: groupSettingWithoutSelf);
    await store.addStream(channel);
  ...
  testWidgets('empty', (tester) async {
    ...
    check(find.widgetWithText(PageBodyEmptyContentPlaceholder,
      'There are no channels in this Zulip organization.')).findsOne();
  });
  ...
  testWidgets('sorting/appearance', (tester) async {
    ...
    final channel8 = await addPrivateChannelWithoutContentAccess('C');
    check(store.streams.length).equals(8);
    await tester.pump();

    final channelsInUiOrder = [channel4, channel5, channel2, channel3,
      channel8, channel7, channel1, channel6];
