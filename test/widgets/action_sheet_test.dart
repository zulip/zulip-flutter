  ...
  group('topic action sheet', () {
    ...
    group('ResolveUnresolveButton', () {
      ...
      testWidgets('resolve: request fails', (tester) async {
        final message = eg.streamMessage(stream: someChannel, topic: 'zulip');
        await prepare(topic: 'zulip');
        await showFromRecipientHeader(tester, topic: 'zulip', message: message);
        connection.takeRequests();
      ...
      Future<void> setupTopicChangeAfterSheetOpened(
        WidgetTester tester, {
        required String oldTopic,
        required String newTopic,
        required String expectedButtonLabel,
      }) async {
        final message = eg.streamMessage(stream: someChannel, topic: oldTopic);
        await prepare(topic: oldTopic);
        await showFromAppBar(tester, topic: oldTopic, messages: [message]);
        connection.takeRequests();
        await store.handleEvent(eg.updateMessageEventMoveFrom(
          origMessages: [message], newTopicStr: newTopic));
        connection.prepare(json: UpdateMessageResult().toJson());

        await tester.tap(findButtonForLabel(expectedButtonLabel));
        await tester.pumpAndSettle();
        check(connection.takeRequests()).isEmpty();
      }

      testWidgets("'zulip' changed to '✔ zulip' while action sheet open, then tap resolve", (tester) async {
        await setupTopicChangeAfterSheetOpened(tester,
          oldTopic: 'zulip',
          newTopic: '✔ zulip',
          expectedButtonLabel: 'Mark as resolved');
        checkErrorDialog(tester,
          expectedTitle: 'Problem marking topic as resolved',
          expectedMessage: 'This topic is already marked as resolved.');
      });

      testWidgets("'zulip' changed to 'asdfjkl;' while action sheet open, then tap resolve", (tester) async {
        await setupTopicChangeAfterSheetOpened(tester,
          oldTopic: 'zulip',
          newTopic: 'asdfjkl;',
          expectedButtonLabel: 'Mark as resolved');
        checkErrorDialog(tester,
          expectedTitle: 'Problem marking topic as resolved',
          expectedMessage: 'This topic has been renamed.');
      });

      testWidgets("'✔ zulip' changed to 'zulip' while action sheet open, then tap unresolve", (tester) async {
        await setupTopicChangeAfterSheetOpened(tester,
          oldTopic: '✔ zulip',
          newTopic: 'zulip',
          expectedButtonLabel: 'Mark as unresolved');
        checkErrorDialog(tester,
          expectedTitle: 'Problem marking topic as unresolved',
          expectedMessage: 'This topic is already marked as unresolved.');
      });

      testWidgets("'✔ zulip' changed to 'asdfjkl;' while action sheet open, then tap unresolve", (tester) async {
        await setupTopicChangeAfterSheetOpened(tester,
          oldTopic: '✔ zulip',
          newTopic: 'asdfjkl;',
          expectedButtonLabel: 'Mark as unresolved');
