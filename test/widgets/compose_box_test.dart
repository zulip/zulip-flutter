...
void main() {
  ...
  group('ComposeBox typing notices', () {
    ...
    testWidgets('clearing text stops typing notice', (tester) async {
      await prepareComposeBox(tester, narrow: narrow, account: account);
    ...
    testWidgets('navigating away sends "stopped typing" notice', (tester) async {
    ...
    testWidgets('for content input, unfocusing sends "stopped typing" notice '
                'and refocusing does not send "started typing notice', (tester) async {
      const narrow = ChannelNarrow(123);
      final destinationNarrow = TopicNarrow(narrow.streamId, 'test topic');
      await prepareComposeBox(tester, narrow: narrow, account: account);
      ...
      await tester.pump(Duration.zero);
      // Refocusing on the content input triggers an update to
      // [TextEditingController.selection].  Still, we expect no
      // "typing started" request because the text remains the same.
    });
    ...
    testWidgets('unfocusing app sends "typing stopped" notice', (tester) async {
      ...
      // Wait for the request to complete, because we cannot use `await` on the
      // triggered `FakeApiConnection.send` call.
      await tester.pump(Duration.zero);
      checkTypingRequest(TypingOp.stop, narrow);
    });
  });
