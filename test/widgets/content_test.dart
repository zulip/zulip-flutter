...
import 'package:zulip/ui/content_block/widgets/block_content_list.dart';
import 'package:zulip/ui/content_block/widgets/global_time.dart';
import 'package:zulip/ui/content_block/widgets/heading.dart';
import 'package:zulip/ui/content_block/widgets/inline_image.dart';
import 'package:zulip/ui/content_block/widgets/mention.dart';
import 'package:zulip/ui/content_block/widgets/message_image_emoji.dart';
import 'package:zulip/ui/content_block/widgets/message_image_preview.dart';
import 'package:zulip/ui/content_block/widgets/thematic_break.dart';
...
TextStyle? mergeSpanStylesOuterToInner(
  ...
  return styles.reversed.reduce(
    (value, element) => switch ((value, element)) {
...
TextStyle? mergedStyleOfSubstring(InlineSpan rootSpan, Pattern spanPattern) {
  return mergeSpanStylesOuterToInner(rootSpan, (span) {
...
TextStyle? mergedStyleOf(
  ...
  if (findAncestor != null) {
    findTextWidget = find.descendant(
      of: findAncestor,
      matching: findTextWidget,
    );
  }
...
void main() {
    ...
    for (final platformRequestsBold in [false, true]) {
      testWidgets(
        ...
        (tester) async {
          ...
          check(style)
            ..fontVariations.isNotNull().any(
              (it) => it
                ..axis.equals('wght')
                ..value.equals(effectiveExpectedWght),
            )
  ...
  group('ThematicBreak', () {
    testWidgets('smoke ThematicBreak', (tester) async {
      await prepareContent(
        tester,
        plainContent(ContentExample.thematicBreak.html),
      );
  ...
  group('ListNodeWidget', () {
    ...
    testWidgets('list uses correct text baseline alignment', (tester) async {
      await prepareContent(
        tester,
        plainContent(ContentExample.orderedListLargeStart.html),
      );
      final table = tester.widget<Table>(find.byType(Table));
      check(
        table.defaultVerticalAlignment,
      ).equals(TableCellVerticalAlignment.baseline);
      check(
        table.textBaseline,
      ).equals(localizedTextBaseline(tester.element(find.byType(Table))));
    });

    testWidgets('ordered list markers have enough space to render completely', (
      tester,
    ) async {
      await prepareContent(
        tester,
      ...
      final marker =
          tester.renderObject(find.textContaining('9999.')) as RenderParagraph;
      // The marker has the height of just one line of text, not more.
      final textHeight = marker.size.height;
      final lineHeight =
          marker.text.style!.height! * marker.text.style!.fontSize!;
  ...
  group('Spoiler', () {
    ...
    group(
      'interactions: spoiler with tappable content (an image) in the header',
      () {
        Future<List<Route<dynamic>>> prepare(
          WidgetTester tester,
          String html,
        ) async {
        ...
        void checkIsExpanded(
          ...
          check(sizeTransition.sizeFactor)
            ...
            ..status.equals(
              expected ? AnimationStatus.completed : AnimationStatus.dismissed,
            );
        }
        ...
        testWidgets('tap image', (tester) async {
          ...
          check(pushedRoutes).single
              .isA<AccountPageRouteBuilder>()
              .fullscreenDialog
              .isTrue(); // recognize the lightbox
        });
        ...
        testWidgets('tap header away from expand/collapse icon (and image)', (
          tester,
        ) async {
          final pushedRoutes = await prepare(tester, example.html);
          checkIsExpanded(tester, false);
          ...
          await tester.tapAt(
            tester.getTopRight(find.byType(RealmContentNetworkImage)) +
                const Offset(10, 0),
          );
          await tester.pumpAndSettle();
          check(pushedRoutes).isEmpty(); // no lightbox
  ...
  group('MessageImagePreview, MessageImagePreviewList', () {
    ...
    group('single image; URLs handled correctly', () {
      ...
      Future<void> doTest(
        ...
        final imageInPreview = tester
            .widgetList<RealmContentNetworkImage>(
              find.descendant(
                of: findImagePreview,
        ...
        ).findsExactly(expectLoadingIndicator ? 1 : 0);

        prepareBoringImageHttpClient();

        final lightboxHeroInPreview = tester
            .widgetList<LightboxHero>(
              find.descendant(
                of: findImagePreview,
                matching: find.byType(LightboxHero),
              ),
            )
            .singleOrNull;
        ...
        if (expectUrlInLightbox != null) {
          check(
            lightboxHeroInPreview,
          ).isNotNull().src.equals(expectUrlInLightbox);
        } else {
        ...
        final lightboxPage = tester
            .widgetList<ImageLightboxPage>(findLightboxPage)
            .singleOrNull;
      ...
      testWidgets('thumbnail', (tester) async {
        ...
        final rawSrc =
            '/user_uploads/thumbnail/2/ce/nvoNL2LaZOciwGZ-FYagddtK/image.jpg/840x560.webp';
        await doTest(
          tester,
          rawHref: rawHref,
          rawSrc: rawSrc,
          ...
          expectThumbnailUrlInLightbox: url(
            '/user_uploads/thumbnail/2/ce/nvoNL2LaZOciwGZ-FYagddtK/image.jpg/840x560.webp',
          ),
      ...
      testWidgets('thumbnail (pre-FL 276)', (tester) async {
        ...
        final rawSrc =
            '/user_uploads/thumbnail/2/c3/wb9FXk8Ej6qIc28aWKcqUogD/image.jpg/840x560.webp';
        await doTest(
          tester,
          rawHref: rawHref,
          rawSrc: rawSrc,
      ...
      testWidgets('thumbnail, animated', (tester) async {
        final rawHref =
            '/user_uploads/2/9f/tZ9c5ZmsI_cSDZ6ZdJmW8pt4/2c8d985d.gif';
        final rawSrc =
            '/user_uploads/thumbnail/2/9f/tZ9c5ZmsI_cSDZ6ZdJmW8pt4/2c8d985d.gif/840x560-anim.webp';
        await doTest(
          tester,
          rawHref: rawHref,
          rawSrc: rawSrc,
      ...
      testWidgets('thumbnail, loading (pre-FL 278)', (tester) async {
        ...
        await doTest(
          ...
          expectUrlInLightbox: url(rawHref),
          example:
              ContentExample.imagePreviewSingleLoadingPlaceholderNoDimensions,
        );
      });

      testWidgets('thumbnail, loading, spinner image itself is a thumbnail', (
        tester,
      ) async {
        final rawHref = '/user_uploads/path/to/spinner.png';
        final rawSrc =
            '/user_uploads/thumbnail/path/to/spinner.png/840x560.webp';
        await doTest(
          tester,
          rawHref: rawHref,
          rawSrc: rawSrc,
          ...
          example: ContentExample
              .imagePreviewSingleLoadingPlaceholderSpinnerIsThumbnail,
        );
      });
      ...
      testWidgets('external; src starts with /external_content', (
        tester,
      ) async {
        final rawHref =
      ...
      testWidgets(
        'external; src starts with https://uploads.zulipusercontent.net/',
        (tester) async {
          final rawHref =
              'https://upload.wikimedia.org/wikipedia/commons/7/78/Verregende_bloem_van_een_Helenium_%27El_Dorado%27._22-07-2023._%28d.j.b%29.jpg';
          final rawSrc =
              'https://uploads.zulipusercontent.';
          await doTest(
            tester,
            rawHref: rawHref,
            rawSrc: rawSrc,
    ...
    testWidgets('multiple images', (tester) async {
      ...
      final expectedImages =
          (example.expectedNodes[1] as ImagePreviewNodeList).imagePreviews;
    ...
    testWidgets('image as immediate child in implicit paragraph', (
      tester,
    ) async {
      const example = ContentExample.imagePreviewInImplicitParagraph;
      await prepare(tester, example.html);
      final expectedImages =
          ((example.expectedNodes[0] as ListNode).items[0][0]
                  as ImagePreviewNodeList)
              .imagePreviews;
      final images = tester.widgetList<RealmContentNetworkImage>(
        find.byType(RealmContentNetworkImage),
      );
    ...
    testWidgets('image cluster in implicit paragraph', (tester) async {
      ...
      await prepare(tester, example.html);
      final expectedImages =
          ((example.expectedNodes[0] as ListNode).items[0][1]
                  as ImagePreviewNodeList)
              .imagePreviews;
      final images = tester.widgetList<RealmContentNetworkImage>(
        find.byType(RealmContentNetworkImage),
  ...
  group("MessageEmbedVideo", () {
    ...
    Future<void> checkEmbedVideo(
      ...
      final expectedTitle =
          (((example.expectedNodes[0] as ParagraphNode).nodes.single
                          as LinkNode)
                      .nodes
                      .single
                  as TextNode)
      ...
      final expectedResolvedUrl = eg.store().tryResolveUrl(
        expectedVideo.previewImageSrcUrl,
      )!;
      ...
      check(testBinding.takeLaunchUrlCalls()).single.equals((
        url: Uri.parse(expectedLaunchUrl),
  ...
  group("CodeBlock", () {
    ...
    testFontWeight(
      'syntax highlighting: non-bold span',
      expectedWght: 400,
  ...
  group('MathBlock', () {
    ...
    testWidgets(
      'fallback to displaying KaTeX source if unsupported KaTeX HTML',
      (tester) async {
        await prepareContent(
          tester,
          plainContent(ContentExample.mathBlockUnknown.html),
        );
  ...
  TargetFontSizeFinder mkTargetFontSizeFinderFromPattern(
  ...
  /// in [GlobalTime].)
  Future<void> checkFontSizeRatio(
    WidgetTester tester, {
    ...
    final headerRootSpan = tester
        .renderObject<RenderParagraph>(find.textContaining('header'))
        .text;
    final headerPlainStyle = mergedStyleOfSubstring(
      headerRootSpan,
      'header-plain ',
    );
    final headerTargetFontSize = targetFontSizeFinder(headerRootSpan);

    final paragraphRootSpan = tester
        .renderObject<RenderParagraph>(find.textContaining('paragraph'))
        .text;
    final paragraphPlainStyle = mergedStyleOfSubstring(
      paragraphRootSpan,
      'paragraph-plain ',
    );
    ...
    check(
      headerPlainStyle!.fontSize! / paragraphPlainStyle!.fontSize!,
    ).isGreaterOrEqual(1.1);

    final ratioInHeader = headerTargetFontSize / headerPlainStyle.fontSize!;
    final ratioInParagraph =
        paragraphTargetFontSize / paragraphPlainStyle.fontSize!;
  ...
  group('strong (bold)', () {
    ...
    testFontWeight(
      'in different kind of span in h1',
      expectedWght: 800,
      // # ~~**bold**~~
    ...
    testFontWeight(
      'in different kind of span in spoiler header',
      expectedWght: 900,
      // ```spoiler *italic **bold***
      // content
      // ```
    ...
    testFontWeight(
      'in different kind of span in table column header',
      expectedWght: 900,
      // | *italic **bold*** |
      // | - |
      // | text |
  ...
  group('inline code', () {
    ...
    testWidgets('maintains font-size ratio with surrounding text', (
      tester,
    ) async {
      await checkFontSizeRatio(
        tester,
    ...
    testWidgets('is link-colored in link span', (tester) async {
      ...
      check(
        style!.color,
      ).equals(const HSLColor.fromAHSL(1, 200, 1, 0.4).toColor());
    });
  });

  group('Mention', () {
    testContentSmoke(
      ContentExample.userMentionPlain,
      wrapWithPerAccountStoreWidget: true,
    );
    testContentSmoke(
      ContentExample.userMentionSilent,
      wrapWithPerAccountStoreWidget: true,
    );
    testContentSmoke(
      ContentExample.userMentionSilentClassOrderReversed,
      wrapWithPerAccountStoreWidget: true,
    );
    testContentSmoke(
      ContentExample.legacyUserMention,
      wrapWithPerAccountStoreWidget: true,
    );
    testContentSmoke(
      ContentExample.groupMentionPlain,
      wrapWithPerAccountStoreWidget: true,
    );
    testContentSmoke(
    ...
    testContentSmoke(
      ContentExample.groupMentionSilentClassOrderReversed,
      wrapWithPerAccountStoreWidget: true,
    );
    testContentSmoke(
      ContentExample.channelWildcardMentionPlain,
      wrapWithPerAccountStoreWidget: true,
    );
    testContentSmoke(
      ContentExample.channelWildcardMentionSilent,
      wrapWithPerAccountStoreWidget: true,
    );
    testContentSmoke(
      ContentExample.channelWildcardMentionSilentClassOrderReversed,
      wrapWithPerAccountStoreWidget: true,
    );
    testContentSmoke(
      ContentExample.legacyChannelWildcardMentionPlain,
      wrapWithPerAccountStoreWidget: true,
    );
    testContentSmoke(
      ContentExample.legacyChannelWildcardMentionSilent,
      wrapWithPerAccountStoreWidget: true,
    );
    testContentSmoke(
      ContentExample.legacyChannelWildcardMentionSilentClassOrderReversed,
      wrapWithPerAccountStoreWidget: true,
    );
    testContentSmoke(
      ContentExample.topicMentionPlain,
      wrapWithPerAccountStoreWidget: true,
    );
    testContentSmoke(
      ContentExample.topicMentionSilent,
      wrapWithPerAccountStoreWidget: true,
    );
    testContentSmoke(
      ContentExample.topicMentionSilentClassOrderReversed,
      wrapWithPerAccountStoreWidget: true,
    );
    ...
    TextStyle textStyleFromWidget(
      WidgetTester tester,
      Mention widget,
      String mentionText,
    ) {
      return mergedStyleOf(
        tester,
        findAncestor: find.byWidget(widget),
        mentionText,
      )!;
    }
    ...
    testWidgets('maintains font-size ratio with surrounding text', (
      ...
      await checkFontSizeRatio(
        tester,
        targetHtml:
            '<span class="user-mention" data-user-id="13313">@Chris Bobbe</span>',
        wrapWithPerAccountStoreWidget: true,
    ...
    testFontWeight(
      'silent or non-self mention in plain paragraph',
      expectedWght: 400,
      // @_**Greg Price**
      content: plainContent(
      ...
      wrapWithPerAccountStoreWidget: true,
      styleFinder: (tester) {
        return textStyleFromWidget(
          tester,
          tester.widget(find.byType(Mention)),
          'Greg Price',
        );
      },
    );
    ...
    testFontWeight(
      'silent or non-self mention in bold context',
      expectedWght: 600,
      // # @_**Chris Bobbe**
      content: plainContent(
    ...
    group('pill color', () {
      Future<void> checkPillColor(
        ...
        final contentTheme = ContentTheme.of(
          tester.element(find.byType(Mention)),
        );
    ...
    group('user mention dynamic name resolution', () {
      ...
      testWidgets('resolves current user name from store', (tester) async {
        await prepare(
          tester: tester,
          html:
              '<p><span class="user-mention" data-user-id="123">@Old Name</span></p>',
          users: [
            eg.selfUser,
            eg.user(userId: 123, fullName: 'New Name'),
          ],
        );
      ...
      testWidgets('falls back to original text when user not found', (
        tester,
      ) async {
        await prepare(
          tester: tester,
      ...
      testWidgets('falls back to original text when userId is null', (
        tester,
      ) async {
        await prepare(
          tester: tester,
      ...
      testWidgets('handles silent mentions correctly', (tester) async {
        await prepare(
          tester: tester,
          html:
              '<p><span class="user-mention silent" data-user-id="123">Old Name</span></p>',
          users: [
            eg.selfUser,
            eg.user(userId: 123, fullName: 'New Name'),
          ],
    ...
    group('user group mention dynamic name resolution', () {
      ...
      testWidgets('resolves current user group name from store', (
        tester,
      ) async {
        await prepare(
          tester: tester,
          html:
              '<p><span class="user-group-mention" data-user-group-id="186">@old-name</span></p>',
      ...
      testWidgets('falls back to original text when user group not found', (
        tester,
      ) async {
        await prepare(
          tester: tester,
          html:
              '<p><span class="user-group-mention" data-user-group-id="999">@Unknown Group</span></p>',
        );
      ...
      testWidgets('handles silent mentions correctly', (tester) async {
        await prepare(
          tester: tester,
          html:
              '<p><span class="user-group-mention silent" data-user-group-id="186">old-name</span></p>',
          userGroups: [eg.userGroup(id: 186, name: 'new-name')],
        );
        check(find.text('new-name')).findsOne();
        check(find.text('@new-name')).findsNothing();
      ...
      for (final (apiName, displayName) in const [
        ...
        testWidgets('shows localized display name for system group $apiName', (
          ...
          await prepare(
            tester: tester,
            html:
                '<p><span class="user-group-mention"'
                ' data-user-group-id="5">@$apiName</span></p>',
            userGroups: [
              eg.userGroup(id: 5, name: apiName, isSystemGroup: true),
            ],
          );
      ...
      testWidgets(
        'shows localized display name for silent system group mention',
        (tester) async {
          await prepare(
            tester: tester,
            html:
                '<p><span class="user-group-mention silent" data-user-group-id="5">role:administrators</span></p>',
            userGroups: [
              eg.userGroup(
                id: 5,
  ...
  group('LinkNode interactions', () {
    ...
    testWidgets(
      'can tap a link to open URL',
      (tester) async {
        await prepare(tester, '<p><a href="https://example/">hello</a></p>');
        ...
        check(testBinding.takeLaunchUrlCalls()).single.equals((
          url: Uri.parse('https://example/'),
          mode: expectedLaunchMode,
        ));
      },
    ...
    testWidgets(
      'follow browser preference setting to open URL',
      (tester) async {
        await testBinding.globalStore.settings.setBrowserPreference(
    ...
    testWidgets('multiple links in paragraph', (tester) async {
      ...
      final base = tester
          .getTopLeft(find.text('foo bar baz'))
    ...
    testWidgets('link containing other spans', (tester) async {
      ...
      final base = tester
          .getTopLeft(find.text('two words'))
          .translate(fontSize / 2, fontSize / 2); // middle of first letter
    ...
    testWidgets('relative links are resolved', (tester) async {
      await prepare(tester, '<p><a href="/a/b?c#d">word</a></p>');
      await tapText(tester, find.text('word'));
      check(testBinding.takeLaunchUrlCalls()).single.equals((
        url: Uri.parse('${eg.realmUrl}a/b?c#d'),
        mode: LaunchMode.inAppBrowserView,
      ));
    });

    testWidgets('link inside HeadingNode', (tester) async {
      await prepare(tester, '<h6><a href="https://a/">word</a></h6>');
      await tapText(tester, find.text('word'));
      check(testBinding.takeLaunchUrlCalls()).single.equals((
        url: Uri.parse('https://a/'),
        mode: LaunchMode.inAppBrowserView,
    ...
    testWidgets('error dialog if invalid URL', (tester) async {
      await prepare(tester, '<p><a href="::invalid::">word</a></p>');
      await tapText(tester, find.text('word'));
      await tester.pump();
      check(testBinding.takeLaunchUrlCalls()).isEmpty();
      checkErrorDialog(
    ...
    testWidgets('error dialog if platform cannot open link', (tester) async {
      await prepare(tester, '<p><a href="file:///etc/bad">word</a></p>');
      testBinding.launchUrlResult = false;
      await tapText(tester, find.text('word'));
      await tester.pump();
      check(testBinding.takeLaunchUrlCalls()).single.equals((
        url: Uri.parse('file:///etc/bad'),
  ...
  group('LinkNode on internal links', () {
    ...
    testWidgets('narrow links are navigated to within app', (tester) async {
      final pushedRoutes = await prepare(
        tester,
      ...
      check(pushedRoutes).single
          ...
          .initNarrow
          .equals(const ChannelNarrow(1));
    });

    // TODO(#1570): test links with /near/ go to the specific message

    testWidgets('uploaded-file links are opened with temporary authed URL', (
      tester,
    ) async {
      final pushedRoutes = await prepare(
        tester,
      ...
      connection.prepare(
        json: GetFileTemporaryUrlResult(url: tempUrlString).toJson(),
      );
  ...
  group('UnicodeEmoji', () {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        check(mergedStyleOf(tester, '\u{2764}')).isNotNull().fontFamily.equals(
    ...
    testWidgets('has strike-through line in strike-through', (tester) async {
      ...
      await prepareContent(
        tester,
        plainContent(
          '<p><del>foo<span aria-label="thumbs up" class="emoji emoji-1f44d" role="img" title="thumbs up">:thumbs_up:</span>bar</del></p>',
        ),
      );
  ...
  group('inline math', () {
    ...
    group('fallback to displaying KaTeX source if unsupported KaTeX HTML', () {
      ...
      testWidgets(
        'maintains font-size ratio with surrounding text, when falling back to TeX source',
        (tester) async {
          await checkFontSizeRatio(
            tester,
      ...
      testWidgets('is link-colored in link span', (tester) async {
        ...
        await prepareContent(
          tester,
          plainContent(
            '<p><a href="https://example/">$unsupportedKatexHtml</a></p>',
          ),
        );
  ...
  group('GlobalTime', () {
    ...
    const timeSpanHtml =
        '<time datetime="2024-01-30T17:33:00Z">2024-01-30T17:33:00Z</time>';
    ...
    final renderedTextRegexp = RegExp(
      r'^(Tue, Jan 30|Wed, Jan 31), 2024, \d+:\d\d(?:\u202F[AP]M)?$',
    );
    final renderedTextRegexpTwelveHour = RegExp(
      r'^(Tue, Jan 30|Wed, Jan 31), 2024, \d+:\d\d\u202F[AP]M$',
    );
    final renderedTextRegexpTwentyFourHour = RegExp(
      r'^(Tue, Jan 30|Wed, Jan 31), 2024, \d+:\d\d$',
    );
    ...
    void testIconAndTextSameColor(String description, String html) {
      testWidgets('clock icon and text are the same color: $description', (
        tester,
      ) async {
        await prepareContent(
          tester,
        ...
        final icon = tester.widget<Icon>(
          find.descendant(
            of: find.byType(GlobalTime),
            matching: find.byIcon(ZulipIcons.clock),
          ),
        );

        final textColor = mergedStyleOf(
          tester,
          findAncestor: find.byType(GlobalTime),
          renderedTextRegexp,
        )!.color;
    ...
    testIconAndTextSameColor(
      'inside link',
      '<p><a href="https://example/">$timeSpanHtml</a></p>',
    );

    group('maintains font-size ratio with surrounding text', () {
      Future<void> doCheck(
        WidgetTester tester,
        double Function(GlobalTime widget) sizeFromWidget,
      ) async {
        await checkFontSizeRatio(
          tester,
          // We use the self-account's time-format setting.
  ...
  group('InlineAudio', () {
    ...
    testWidgets(
      'tapping on audio link opens it in browser',
      (tester) async {
        ...
        connection.prepare(
          json: GetFileTemporaryUrlResult(url: url.path).toJson(),
        );
  ...
  group('MessageImageEmoji', () {
    ...
    testWidgets('smoke: custom emoji with invalid URL', (tester) async {
      ...
      final url = tester
          .widget<MessageImageEmoji>(find.byType(MessageImageEmoji))
          .node
          .src;
  ...
  group('InlineImage', () {
    ...
    testWidgets('smoke: inline image', (tester) async {
      ...
      ).findsOne(); // recognize the lightbox
      debugNetworkImageHttpClientProvider = null;
    });
  ...
  group('WebsitePreview', () {
    ...
    testWidgets('smoke: without title', (tester) async {
      final url = Uri.parse(
        ContentExample.websitePreviewWithoutTitle.markdown!,
      );
    ...
    testWidgets('smoke: without description', (tester) async {
      final url = Uri.parse(
        ContentExample.websitePreviewWithoutDescription.markdown!,
      );
      await prepare(
        tester,
        ContentExample.websitePreviewWithoutDescription.html,
      );
    ...
    testWidgets('smoke: without title or description', (tester) async {
      final url = Uri.parse(
        ContentExample.websitePreviewWithoutTitleOrDescription.markdown!,
      );
      await prepare(
        tester,
        ContentExample.websitePreviewWithoutTitleOrDescription.html,
      );
  ...
  group('MessageTable', () {
    ...
    testWidgets('header row background color', (tester) async {
      await prepareContent(
        tester,
        plainContent(ContentExample.tableWithSingleRow.html),
      );
      final BuildContext context = tester.element(find.byType(Table));
      check(tester.widget<Table>(find.byType(Table))).children.first.decoration
          .isA<BoxDecoration>()
          .color
          .equals(ContentTheme.of(context).colorTableHeaderBackground);
    });
    ...
    testWidgets('different text alignment in columns', (tester) async {
      await prepareContent(
        ...
        plainContent(
          ContentExample.tableWithDifferentTextAlignmentInColumns.html,
        ),
      );

      final defaultAlignedText = tester.renderObject<RenderParagraph>(
        find.textContaining('default-aligned'),
      );
      check(defaultAlignedText.textAlign).equals(TextAlign.start);

      final leftAlignedText = tester.renderObject<RenderParagraph>(
        find.textContaining('left-aligned'),
      );
      check(leftAlignedText.textAlign).equals(TextAlign.left);

      final centerAlignedText = tester.renderObject<RenderParagraph>(
        find.textContaining('center-aligned'),
      );
      check(centerAlignedText.textAlign).equals(TextAlign.center);

      final rightAlignedText = tester.renderObject<RenderParagraph>(
        find.textContaining('right-aligned'),
      );
    ...
    testWidgets('text alignment in column; with link', (tester) async {
      ...
      final linkText = tester.renderObject<RenderParagraph>(
        find.textContaining('https://zulip.com'),
      );
