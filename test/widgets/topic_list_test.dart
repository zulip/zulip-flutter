...
void main() {
  ...
  // Enough topics that the list scrolls.
  final topicsAtoZ = [
    for (int i = 0; i < 26; i++)
      eg.getChannelTopicsEntry(
        name: 'topic ${String.fromCharCode('A'.codeUnitAt(0) + i)}',
        maxId: 26 - i),
  ];

  void checkTopicsVisible({required bool first, required bool last}) {
    check(findInTopicItem(find.text('topic A'))).findsExactly(first ? 1 : 0);
    check(findInTopicItem(find.text('topic Z'))).findsExactly(last ? 1 : 0);
  }

  Future<void> scrollToEnd(WidgetTester tester) async {
    await tester.dragUntilVisible(
      findInTopicItem(find.text('topic Z')),
      find.byType(ListView),
      Offset(0, -50));
    await tester.pump();
  }

  testWidgets('tap status bar to scroll to top', (tester) async {
    addTearDown(tester.view.reset);
    tester.view.padding = FakeViewPadding(top: 25); // the status bar

    await prepare(tester, topics: topicsAtoZ);
    checkTopicsVisible(first: true, last: false);

    await scrollToEnd(tester);
    checkTopicsVisible(first: false, last: true);
  ...
  group('filter topics field', () {
    ...
    testWidgets('shows empty state when no filtered topics', (tester) async {
      ...
      await tester.enterText(find.byType(TextField), 'channelsss');
      await tester.pump();
      ...
      check(findInTopicItem(find.text('topic A'))).findsOne();
      check(findInTopicItem(find.text('topic B'))).findsOne();
      check(find.text('No topics match your filters.')).findsNothing();
    });

    testWidgets('scroll to top on query change', (tester) async {
      await prepare(tester, topics: topicsAtoZ);
      checkTopicsVisible(first: true, last: false);
