  ...
  group('MarkAsReadWidget', () {
    ...
    group('confirmation dialog', () {
      ...
      testWidgets('CombinedFeedNarrow: show dialog', (tester) async {
        final messages = List.generate(25,
          (_) => eg.streamMessage(flags: []));
        await showsConfirmDialog(tester,
          narrow: CombinedFeedNarrow(),
          messages: [...messages],
          unreadMsgs: eg.unreadMsgs(channels: [
            UnreadChannelSnapshot(
              streamId: messages.first.streamId,
              topic: messages.first.topic,
              unreadMessageIds: messages.map((message) => message.id).toList()),
          ]));
      });

      testWidgets('MentionsNarrow: show dialog', (tester) async {
        final messages = List.generate(25,
          (_) => eg.streamMessage(flags: [MessageFlag.mentioned]));
        await showsConfirmDialog(tester,
          narrow: MentionsNarrow(),
          messages: [...messages],
          unreadMsgs: eg.unreadMsgs(
            mentions: messages.map((message) => message.id).toList(),
            channels: [
              UnreadChannelSnapshot(
                streamId: messages.first.streamId,
                topic: messages.first.topic,
      ...
      testWidgets('ChannelNarrow: show dialog', (tester) async {
        final messages = List.generate(25,
          (_) => eg.streamMessage(flags: []));
        await showsConfirmDialog(tester,
          narrow: ChannelNarrow(messages.first.streamId),
          messages: [...messages],
          unreadMsgs: eg.unreadMsgs(channels: [
            UnreadChannelSnapshot(
              streamId: messages.first.streamId,
      ...
      testWidgets('DmNarrow: do not show dialog', (tester) async {
        final messages = List.generate(25,
          (_) => eg.dmMessage(flags: [], from: eg.selfUser, to: [eg.otherUser]));
        await doesNotShowConfirmDialog(tester,
          narrow: DmNarrow.ofMessage(messages.first, selfUserId: eg.selfUser.userId),
          messages: [...messages],
          ...
          updateResult: UpdateMessageFlagsForNarrowResult(
            ...
            firstProcessedId: messages.first.id, lastProcessedId: messages.first.id,
      ...
      testWidgets('TopicNarrow: do not show dialog', (tester) async {
        final messages = List.generate(25,
          (_) => eg.streamMessage(flags: []));
        await doesNotShowConfirmDialog(tester,
          narrow: TopicNarrow.ofMessage(messages.first),
          messages: [...messages],
          unreadMsgs: eg.unreadMsgs(channels: [
            UnreadChannelSnapshot(topic: messages.first.topic,
              streamId: messages.first.streamId,
              unreadMessageIds: messages.map((message) => message.id).toList())]),
          updateResult: UpdateMessageFlagsForNarrowResult(
            processedCount: 1, updatedCount: 1,
