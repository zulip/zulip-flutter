...
void main() {
  ...
  Future<void> setupMessageListPage(
    WidgetTester tester, {
    ...
    streams ??=
        subscriptions ??= [
          eg.subscription(eg.stream(streamId: eg.defaultStreamMessageStreamId)),
        ];
  ...
  group('MessageListPage', () {
    testWidgets('ancestorOf finds page state from message', (tester) async {
      ...
      check(
        MessageListPage.ancestorOf(tester.element(find.text("a message"))),
      ).identicalTo(expectedState as MessageListPageState);
    });

    testWidgets('ancestorOf throws when not a descendant of MessageListPage', (
      tester,
    ) async {
      await setupMessageListPage(
        tester,
      ...
      check(() => MessageListPage.ancestorOf(element)).throws<void>();
    });
    ...
    testWidgets('MessageListPageState.narrow', (tester) async {
      ...
      await setupMessageListPage(
        ...
        streams: [stream],
        messages: [
          eg.streamMessage(stream: stream, content: "<p>a message</p>"),
        ],
      );
    ...
    testWidgets('composeBoxController null when no compose box', (
      tester,
    ) async {
      await setupMessageListPage(
        tester,
    ...
    testWidgets('dispose MessageListView when event queue expired', (
      tester,
    ) async {
      ...
      updateMachine.debugPrepareLoopError(
        ZulipApiException(
          ...
          data: {'queue_id': updateMachine.queueId},
          message: 'Bad event queue ID.',
        ),
      );
      ...
      // rebuilt yet.
      final newStore =
          testBinding.globalStore.perAccountSync(eg.selfAccount.id)!;
      check(
        connection.isOpen,
      ).isFalse(); // indicates that the old store has been disposed
      check(store.debugMessageListViews).single.equals(oldViewModel);
      check(newStore.debugMessageListViews).isEmpty();

      (newStore.connection as FakeApiConnection).prepare(
        json:
            eg
      ...
      check(
        newStore.debugMessageListViews,
      ).single.not((it) => it.equals(oldViewModel));
    });
  ...
  group('app bar', () {
    ...
    testWidgets('has channel-feed action for topic narrows', (tester) async {
      ...
      await setupMessageListPage(
        tester,
        narrow: eg.topicNarrow(channel.streamId, 'hi'),
    ...
    testWidgets('show topic visibility policy for topic narrows', (
      tester,
    ) async {
      final channel = eg.stream();
      const topic = 'topic';
      ...
      check(
        find.descendant(
          ...
          matching: find.byIcon(ZulipIcons.mute),
        ),
      ).findsOne();
    });
  });
  ...
  group('presents message content appropriately', () {
    testWidgets(
      'content not asked to consume insets (including bottom), even without compose box',
      (tester) async {
        // Regression test for: https://github.com/zulip/zulip-flutter/issues/736
        const fakePadding = FakeViewPadding(
          left: 10,
          top: 10,
          right: 10,
          bottom: 10,
  ...
  testWidgets('smoke test for light/dark/lerped', (tester) async {
    ...
    Color backgroundColor() {
      final coloredBoxFinder = find.descendant(
        of: find.byWidgetPredicate(
          (w) => w is MessageItem && w.item.message.id == message.id,
        ),
    ...
    check(
      backgroundColor(),
    ).isSameColorAs(MessageListTheme.light.streamMessageBgDefault);

    tester.platformDispatcher.platformBrightnessTestValue = Brightness.dark;
    await tester.pump();

    await tester.pump(kThemeAnimationDuration * 0.4);
    final expectedLerped = MessageListTheme.light.lerp(
      MessageListTheme.dark,
      0.4,
    );
    check(
      backgroundColor(),
    ).isSameColorAs(expectedLerped.streamMessageBgDefault);

    await tester.pump(kThemeAnimationDuration * 0.6);
    check(
      backgroundColor(),
    ).isSameColorAs(MessageListTheme.dark.streamMessageBgDefault);
  });
    ...
    testWidgets('basic', (tester) async {
      await setupMessageListPage(
        ...
        messages: List.generate(
          300,
          (i) => eg.streamMessage(id: 950 + i, sender: eg.selfUser),
        ),
      );
      ...
      connection.prepare(
        json:
            eg
                .olderGetMessagesResult(
                  anchor: 950,
                  foundOldest: false,
                  messages: List.generate(
                    100,
                    (i) => eg.streamMessage(id: 850 + i, sender: eg.selfUser),
                  ),
                )
                .toJson(),
      );
      ...
      ); // Fast-forward to end of fling.
      await tester.pump(
        Duration.zero,
      ); // Allow a frame for the response to arrive.
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        for (int i = 0; i < 30; i++) {
          ...
          if (itemCount(tester)! > 101)
            break; // The loading indicator appeared.
        }
        await tester.pump(
          Duration.zero,
        ); // Allow a frame for the response to arrive.
          ...
          json:
              eg
                  .olderGetMessagesResult(
                    ...
                    messages: List.generate(
                      100,
                      (i) => eg.streamMessage(id: 750 + i, sender: eg.selfUser),
                    ),
                  )
                  .toJson(),
        );
      ...
      skip: true,
    ); // TODO this still reproduces manually, still needs debugging,
    // but has become harder to reproduce in a test.

    testWidgets("avoid getting distracted by nested viewports' metrics", (
      tester,
    ) async {
      ...
      await setupMessageListPage(
        ...
        messages: [
          ...
          eg.streamMessage(
            id: 1301,
            content: ContentExample.codeBlockPlain.html,
          ),
  ...
  group('TypingStatusWidget', () {
    ...
    final dmMessage = eg.dmMessage(
      from: eg.selfUser,
      to: [eg.otherUser, eg.thirdUser, eg.fourthUser],
    );
    final dmNarrow = DmNarrow.ofMessage(
      dmMessage,
  ...
  group('MarkAsReadWidget', () {
    bool isMarkAsReadButtonVisible(WidgetTester tester) {
      ...
      final finder =
          find.text(zulipLocalizations.markAllAsReadLabel).hitTestable();
    ...
    testWidgets('from read to unread', (tester) async {
      ...
      await store.handleEvent(
        eg.updateMessageFlagsRemoveEvent(MessageFlag.read, [message]),
      );
      await tester.pumpAndSettle();
    ...
    group('onPressed behavior', () {
      ...
      group('MarkAsReadAnimation', () {
        ...
        testWidgets('loading is changed correctly if request fails', (
          tester,
        ) async {
          final narrow = TopicNarrow.ofMessage(message);
          await setupMessageListPage(
      ...
      testWidgets('smoke test on modern server', (tester) async {
        ...
        await tester.tap(find.byType(MarkAsReadWidget));
        final apiNarrow =
            narrow.apiEncode()..add(ApiNarrowIs(IsOperand.unread));
        ...
        connection.prepare(
          json:
              UpdateMessageFlagsForNarrowResult(
                processedCount: 20,
                updatedCount: 10,
                firstProcessedId: 2000,
                lastProcessedId: 2023,
                foundOldest: false,
                foundNewest: true,
              ).toJson(),
        );
      ...
      testWidgets(
        'markNarrowAsRead on mark-all-as-read when Unreads.oldUnreadsMissing: true',
        (tester) async {
  ...
  group('Update Narrow on message move', () {
    ...
    void prepareGetMessageResponse(List<Message> messages) {
      connection.prepare(
        json:
            eg
                .newestGetMessagesResult(foundOldest: false, messages: messages)
                .toJson(),
      );
    }

    void handleMessageMoveEvent(
    ...
    testWidgets('compose box send message after move', (tester) async {
      final message = eg.streamMessage(
        stream: channel,
        topic: topic,
        content: 'Message to move',
      );
      await setupMessageListPage(
        tester,
        narrow: narrow,
        messages: [message],
        streams: [channel, otherChannel],
      );
      ...
      check(tester.widget<TextField>(channelContentInputFinder))
        ..decoration.isNotNull().hintText.equals(
          'Message #${channel.name} > $topic',
        )
        ..controller.isNotNull().text.equals('Some text');

      prepareGetMessageResponse([message]);
      handleMessageMoveEvent(
        [message],
        'new topic',
        newChannelId: otherChannel.streamId,
      );
      await tester.pump(const Duration(seconds: 1));
      check(tester.widget<TextField>(channelContentInputFinder))
        ..decoration.isNotNull().hintText.equals(
          'Message #${otherChannel.name} > new topic',
        )
        ..controller.isNotNull().text.equals('Some text');
    ...
    testWidgets('Move to narrow with existing messages', (tester) async {
      ...
      check(
        find.textContaining('Existing message').evaluate(),
      ).length.equals(0);
  ...
  group('recipient headers', () {
    group('StreamMessageRecipientHeader', () {
      ...
      FinderResult<Element> findInMessageList(String text) {
        ...
        return find
            .descendant(of: find.byType(MessageList), matching: find.text(text))
            .evaluate();
      }
      ...
      testWidgets('show topic visibility icon when followed', (tester) async {
        ...
        check(
          find.descendant(
            ...
            matching: find.byIcon(ZulipIcons.follow),
          ),
        ).findsOne();
      });
      ...
      testWidgets('show topic visibility icon when unmuted', (tester) async {
        ...
        check(
          find.descendant(
            of: find.byType(MessageList),
            matching: find.byIcon(ZulipIcons.unmute),
          ),
        ).findsOne();
      });
      ...
      testWidgets('color of stream icon', (tester) async {
        ...
        await tester.pump();
        check(
          tester.widget<Icon>(find.byIcon(ZulipIcons.globe)),
      ...
      testWidgets(
        'show stream name from message when stream unknown',
        (tester) async {
        ...
        skip: true,
      ); // TODO(#252) could repro this with search narrows, once we have those

      testWidgets('show stream name from stream data when known', (
        tester,
      ) async {
        ...
        await setupMessageListPage(
          ...
          messages: [eg.streamMessage(stream: streamBefore)],
        );
        await tester.pump();
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          check(pushedRoutes).single
              ...
              .isA<MessageListPage>()
              .initNarrow
              .equals(TopicNarrow.ofMessage(message));
          await tester.pumpAndSettle();
        },
      );

      testWidgets('does not navigate on tapping topic in TopicNarrow', (
        tester,
      ) async {
        final pushedRoutes = <Route<void>>[];
        final navObserver =
    ...
    group('DmRecipientHeader', () {
      testWidgets('show names', (tester) async {
        ...
        await tester.pump();
        tester.widget(
          find.text(zulipLocalizations.messageListGroupYouWithYourself),
        );
        tester.widget(
          find.text(
      ...
      testWidgets('icon color matches text color', (tester) async {
        ...
        check(
          textSpan,
        ).style.isNotNull().color.isNotNull().isSameColorAs(icon.color!);
      });
    });
    ...
    testWidgets(
      'navigates to DmNarrow on tapping recipient header in CombinedFeedNarrow',
      (tester) async {
        final pushedRoutes = <Route<void>>[];
        final navObserver =
            TestNavigatorObserver()
    ...
    testWidgets('does not navigate on tapping recipient header in DmNarrow', (
      tester,
    ) async {
      final pushedRoutes = <Route<void>>[];
      final navObserver =
    ...
    testWidgets('Updates avatar on RealmUserUpdateEvent', (tester) async {
      ...
      void checkResultForSender(String? avatarUrl) {
        if (avatarUrl == null) {
          ...
          check(
            findAvatarImageWidget(tester),
          ).isNotNull().src.equals(eg.selfAccount.realmUrl.resolve(avatarUrl));
        }
      }

      Future<void> handleNewAvatarEventAndPump(
        WidgetTester tester,
        String avatarUrl,
      ) async {
        await store.handleEvent(
    ...
    testWidgets('Bot user is distinguished by showing an icon', (tester) async {
      ...
      void checkUser(User user, {required bool isBot}) {
        ...
        final userFinder = find.ancestor(
          of: nameFinder,
          matching: find.ancestor(of: botFinder, matching: find.byType(Row)),
        );
  ...
  group('edit state label', () {
    ...
    testWidgets('edited and moved messages from events', (tester) async {
      ...
      await store.handleEvent(
        eg.updateMessageEditEvent(message, renderedContent: 'edited'),
      );
      await tester.pump();
      ...
      await store.handleEvent(
        eg.updateMessageEditEvent(message2, renderedContent: 'edited'),
      );
      await tester.pump();
