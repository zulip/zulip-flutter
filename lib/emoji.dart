...
final emojiSet = defaultEmojiSet.map((emojiCategory) =>
    CategoryEmoji(
      emojiCategory.category,
      emojiCategory.emoji
        .where((emoji) => emojiNameMaps[getEmojiCode(emoji)] != null)
        .map((emoji) =>
...
String getEmojiCanonicalName(Emoji emoji) {
  ...
  if (emojiName == null) {
    throw Exception("Emoji code $emojiCode not found in emojiNameMaps. Please add it to emojiNameMaps.");
  }
  final canonicalName = emojiName["canonical_name"];
  if (canonicalName is String) {
    return canonicalName;
  }
  throw Exception("Emoji code $emojiCode not found in emojiNameMaps. Please add it to emojiNameMaps.");
}

String getEmojiCode(Emoji emoji) {
  final emojiString = emoji.emoji;
  final codeUnits = emojiString.runes.toList();
  final emojiCode = codeUnits
    // unqualify_emoji
    // See https://github.com/zulip/zulip/blob/f4d58f1ba6ab612610a261dda7a875006e8b0c17/zerver/lib/emoji_utils.py#L4
    .where((codeUnit) => codeUnit != 0xfe0f)
    .map((codeUnit) => codeUnit.toRadixString(16).padLeft(4, '0')).join();
  return emojiCode;
}

/// A map of emoji code to canonical name and aliases.
/// From https://github.com/zulip/zulip/blob/f4d58f1ba6ab612610a261dda7a875006e8b0c17/tools/setup/emoji/emoji_names.py
final emojiNameMaps = {
