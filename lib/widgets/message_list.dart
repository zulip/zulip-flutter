...
class MessageListPage extends StatefulWidget {
  ...
  static RevealedMutedMessagesState? maybeRevealedMutedMessagesOf(
    ...
    final state = context
        .dependOnInheritedWidgetOfExactType<_RevealedMutedMessagesProvider>()
        ?.state;
    return state;
  }
...
class _MessageListPageState extends State<MessageListPage>
    implements MessageListPageState {
  @override
  late Narrow narrow;
  ...
  Widget build(BuildContext context) {
    ...
    result = _RevealedMutedMessagesProvider(
...
class _RevealedMutedMessagesProvider
        ...
        return Column(
          ...
          children: [
            ...
            GestureDetector(
              behavior: HitTestBehavior.translucent,
              onLongPress: () {
                final someMessage = MessageListPage.ancestorOf(
                  context,
...
class _MarkAsReadWidgetState extends State<MarkAsReadWidget> {
  ...
  void _handlePress(BuildContext context) async {
    ...
    // Get count of visible (unmuted) unreads
    final visibleUnreadCount = store.unreads.countInNarrow(widget.narrow);
    // Get count of all unreads including muted
    final totalUnreadCount = store.unreads.countInNarrowIncludingMuted(
      widget.narrow,
    );
    final mutedUnreadCount = totalUnreadCount - visibleUnreadCount;

    // Show confirmation dialog with appropriate message
    final String message;
    if (mutedUnreadCount > 0) {
      message = zulipLocalizations.markAsReadConfirmationDialogMessageWithMuted(
        totalUnreadCount,
        mutedUnreadCount,
      );
    } else {
      message = zulipLocalizations.markAsReadConfirmationDialogMessage(
        totalUnreadCount,
      );
    }

    final dialogResult = await showSuggestedActionDialog(
      context: context,
      title: zulipLocalizations.markAllAsReadLabel,
      message: message,
      actionButtonText:
          zulipLocalizations.markAsReadConfirmationDialogConfirmButton,
    );

    // Check if user confirmed
    if (dialogResult.result case final confirmed? when confirmed == true) {
      if (!context.mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    if (editMessageErrorStatus != null) {
      ...
      if (!editMessageErrorStatus) {
        ...
        content = _RestoreEditMessageGestureDetector(
...
class _EditMessageStatusRow extends StatelessWidget {
  const _EditMessageStatusRow({required this.messageId, required this.status});
...
class _RestoreEditMessageGestureDetector extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      onTap: () {
        final composeBoxState = MessageListPage.ancestorOf(
          context,
        ).composeBoxState;
