...
class _SetStatusPageState extends State<SetStatusPage> {
  ...
  List<UserStatus> statusSuggestions(BuildContext context) {
    ...
    final values = [
      ...
      (localizations.userStatusInAMeeting,      '1f4c5', 'calendar'),
      (localizations.userStatusCommuting,       '1f68c', 'bus'),
      (localizations.userStatusOutSick,         '1f912', 'sick'),
      (localizations.userStatusVacationing,     '1f334', 'palm_tree'),
      (localizations.userStatusWorkingRemotely, '1f3e0', 'house'),
      (localizations.userStatusAtTheOffice,     '1f3e2', 'office'),
    ];

    return [
      for (final (statusText, emojiCode, fallbackEmojiName) in values)
        UserStatus(
          text: statusText,
          emoji: StatusEmoji(
            emojiName: store.allEmojiCandidates()
                         .firstWhereOrNull((e) => e.emojiCode == emojiCode)
                         ?.emojiName ?? fallbackEmojiName,
            emojiCode: emojiCode,
  ...
  Future<void> handleStatusSave() async {
    ...
    try {
      await updateStatus(store.connection, status: newStatus);
    } catch (e) {
