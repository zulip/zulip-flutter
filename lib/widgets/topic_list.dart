...
class _TopicListState extends State<_TopicList> with PerAccountStoreAwareStateMixin {
  ...
  void initState() {
    super.initState();
    _searchController = TextEditingController()..addListener(_handleControllerUpdate);
  }

  @override
  void onNewStore() {
    final newStore = PerAccountStoreWidget.of(context);
    final query = TopicAutocompleteQuery(_searchController.text);
  ...
  void _handleControllerUpdate() {
    final oldQuery = _topicAutocompleteModel!.query;
    final newQuery = TopicAutocompleteQuery(_searchController.text);
    if (newQuery == oldQuery) return;
    _topicAutocompleteModel!.query = newQuery;
  ...
  Widget build(BuildContext context) {
    ...
    // If the fetch succeeds, `_topicAutocompleteModel` will notify listeners.
    ...
    if (_searchController.text.isEmpty && topics!.isEmpty) {
    ...
    return SafeArea(
      ...
      child: Column(
        children: [
          _FilterTopicsField(controller: _searchController),
          Expanded(child: topicItems.isEmpty
            ? PageBodyEmptyContentPlaceholder(header: zulipLocalizations.filteredTopicsEmptyPlaceholderHeader)
            : ListView.builder(
                // Use the enclosing route's [PrimaryScrollController], so that
                // tapping the status bar on iOS scrolls this list to the top.
                primary: true,
