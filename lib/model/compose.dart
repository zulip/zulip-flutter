...
final _channelTopicAvoidedCharsReplacementsInverse =
  _channelTopicAvoidedCharsReplacements.map((k, v) => MapEntry(v, k));
...
String unescapeChannelTopicAvoidedChars(String str) {
  return str.replaceAllMapped(_channelTopicAvoidedCharsReplacementsRegex,
    (match) => _channelTopicAvoidedCharsReplacementsInverse[match[0]]!);
}

/// Markdown link for a channel or topic whose name includes characters that
...
String _channelTopicFallbackMarkdownLink(PerAccountStore store, {required ZulipStream channel, TopicName? topic}) {
...
String channelLink(ZulipStream channel, {
  bool pendingTopicAutocomplete = false,
  required PerAccountStore store,
}) {
  if (_channelTopicAvoidedCharsRegex.hasMatch(channel.name)) {
    return '${_channelTopicFallbackMarkdownLink(channel: channel, store)}${pendingTopicAutocomplete ? '>' : ''}';
  }
  return '#**${channel.name}${pendingTopicAutocomplete ? '>' : '**'}';
}

/// A #channel>topic link syntax of a topic, like #**announce>GSoC**.
///
/// A plain Markdown link will be used if either the channel or topic name
/// includes some characters that would break normal #**channel>topic** rendering.
String topicLink(ZulipStream channel, TopicName topic, {required PerAccountStore store}) {
  if (_channelTopicAvoidedCharsRegex.hasMatch(channel.name)
    ...
    return _channelTopicFallbackMarkdownLink(channel: channel, topic: topic, store);
  }
