...
private func doubleEqualsNotifications(_ lhs: Double, _ rhs: Double) -> Bool {
  return (lhs.isNaN && rhs.isNaN) || lhs == rhs
}

private func doubleHashNotifications(_ value: Double, _ hasher: inout Hasher) {
  if value.isNaN {
...
func deepEqualsNotifications(_ lhs: Any?, _ rhs: Any?) -> Bool {
  ...
  case (let lhsArray, let rhsArray) as ([Any?], [Any?]):
    ...
    for (index, element) in lhsArray.enumerated() {
      if !deepEqualsNotifications(element, rhsArray[index]) {
        return false
      }
    }
    return true
  ...
  case (let lhsArray, let rhsArray) as ([Double], [Double]):
    guard lhsArray.count == rhsArray.count else { return false }
    for (index, element) in lhsArray.enumerated() {
      if !doubleEqualsNotifications(element, rhsArray[index]) {
        return false
      }
    }
    return true
  ...
  case (let lhsDictionary, let rhsDictionary) as ([AnyHashable: Any?], [AnyHashable: Any?]):
    ...
    for (lhsKey, lhsValue) in lhsDictionary {
      ...
      for (rhsKey, rhsValue) in rhsDictionary {
        if deepEqualsNotifications(lhsKey, rhsKey) {
          if deepEqualsNotifications(lhsValue, rhsValue) {
            found = true
            break
          } else {
            return false
  ...
  case (let lhs as Double, let rhs as Double):
    return doubleEqualsNotifications(lhs, rhs)
...
func deepHashNotifications(value: Any?, hasher: inout Hasher) {
  ...
  if let cleanValue = cleanValue {
    if let doubleValue = cleanValue as? Double {
      doubleHashNotifications(doubleValue, &hasher)
    } else if let valueList = cleanValue as? [Any?] {
      for item in valueList {
        deepHashNotifications(value: item, hasher: &hasher)
      }
    } else if let valueList = cleanValue as? [Double] {
      for item in valueList {
        doubleHashNotifications(item, &hasher)
      }
    } else if let valueDict = cleanValue as? [AnyHashable: Any?] {
      var result = 0
      for (key, value) in valueDict {
        var entryKeyHasher = Hasher()
        deepHashNotifications(value: key, hasher: &entryKeyHasher)
        var entryValueHasher = Hasher()
        deepHashNotifications(value: value, hasher: &entryValueHasher)
...
struct NotificationDataFromLaunch: Hashable {
  ...
  static func == (lhs: NotificationDataFromLaunch, rhs: NotificationDataFromLaunch) -> Bool {
    ...
    return deepEqualsNotifications(lhs.payload, rhs.payload)
  }

  func hash(into hasher: inout Hasher) {
    hasher.combine("NotificationDataFromLaunch")
    deepHashNotifications(value: payload, hasher: &hasher)
  }
}
...
struct IosNotificationTapEvent: NotificationTapEvent {
  ...
  func hash(into hasher: inout Hasher) {
    hasher.combine("IosNotificationTapEvent")
    deepHashNotifications(value: payload, hasher: &hasher)
  }
}
...
struct AndroidNotificationTapEvent: NotificationTapEvent {
  ...
  static func == (lhs: AndroidNotificationTapEvent, rhs: AndroidNotificationTapEvent) -> Bool {
    ...
    return deepEqualsNotifications(lhs.dataUrl, rhs.dataUrl)
  }

  func hash(into hasher: inout Hasher) {
    hasher.combine("AndroidNotificationTapEvent")
    deepHashNotifications(value: dataUrl, hasher: &hasher)
  }
}
